...
module.exports.config = { 
    name: 'sicil',
    aliases: ['ceza-puan','cezapuan', 'cezalar', 'puan', 'sicil', 'cpuan']
}

module.exports.raviwen = async(client, message, args, config) => {
    let embed = new MessageEmbed().setAuthor(message.author.tag, message.author.displayAvatarURL()).setColor("RANDOM")
    ...
    let sicil = data.length > 0 ? data.map((value, index) => `\`${index+1}.\`**[${value.Komut}]** ${client.users.cache.get(value.Yetkili) || value.Yetkili} tarafından **${value.Sebep}** sebebiyle cezalandırılmış.  `).join("\n") : "Bu Üyenin Ceza Bilgisi Bulunamadı."
